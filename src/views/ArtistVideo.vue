    ...
    <div class="mt-5">
        <div class="grid grid-flow-row gap-5 grid-cols-2 lg:grid-cols-3 xl:grid-cols-4  2xl:grid-cols-5">
            <div v-for="(item, index) in list" :key="index" :class="{ 'item-1': index === 0 }"
                ...
                <CoverPlay :name="item.name" :pic-url="item.imgurl16v9" video :play-count="item.playCount"
                    show-play-count />
        ...
        <div class="flex justify-center py-5" v-if="list.length > 0 && !pageData.noMore">
            <el-button :loading="pageData.loading" type="text" class="text-center  w-full"
...
import { useArtistMv, useSearchVideo } from "@/api/api";
...
const getData = async () => {
    try {
        ...
        let mvs = new Array<Mv>()
        if (props.id) {
            mvs = (await useArtistMv(props.id, pageData.limit, offset.value)).mvs
        } else if (props.keywords) {
            mvs = (await useSearchVideo(props.keywords))
        }
