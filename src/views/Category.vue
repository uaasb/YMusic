<template>
  ...
  <div class="gap-5  grid grid-flow-row grid-cols-3 lg:grid-cols-5 2xl:grid-cols-7">
    <div v-for="(item, index) in list" :key="index" :class="{ 'item-1': index === 0 }"
      ...
      <CoverPlay :name="item.name" :pic-url="item.coverImgUrl" class="pointer" :play-count="item.playCount"
        show-play-count />
      <div class="mt-2 text-xs text-main  leading-5 pointer truncate">{{ item.name }}</div>
      <div class="mt-2 text-xs text-main  truncate hover:text-brandColor pointer truncate">{{ item.creator.nickname }}
      </div>
    </div>
  </div>
...
import PlaylistHot from "@/views/PlaylistHot.vue";
