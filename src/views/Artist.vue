...
import { useSearchArtist, userArtistList } from "@/api/api";
...
const getData = async () => {
  ...
  try {
    if (props.keywords) {
      artistlist = await useSearchArtist(props.keywords)
    } else {
      artistlist = await userArtistList(pageData)
    }

    if (pageData.page === 1) {
      artists.value = artistlist
    } else {
      artists.value.push(...artistlist)
    }
    pageData.init = true
